...
factors of 1, 2, 3 and 4, respectively. You are going to answer several queries of the form: What is the minimal impact factor of nucleotides
...
that, given a non-empty string S consisting of N characters and two non-empty arrays P and Q consisting of M integers, returns an array consisting
...
*/
func GenRanQ(S string, P []int, Q []int) []int {
	const A, C, G, T = 1, 2, 3, 4
	valsS := make([]int, len(S))
	aList := make([]int, len(S))
	cList := make([]int, len(S))
	gList := make([]int, len(S))
	tList := make([]int, len(S))
	a, c, g, t := 0, 0, 0, 0
	for i := 0; i < len(S); i++ {
		switch S[i] {
		case 'A':
		...
		aList[i], cList[i], gList[i], tList[i] = a, c, g, t
	}
	...
	for i := 0; i < len(P); i++ {
		...
		if k1 == k2 {
			...
			switch true {
			case aList[k1] < aList[k2] || valsS[k1] == A:
				result[i] = A
			case cList[k1] < cList[k2] || valsS[k1] == C:
				result[i] = C
			case gList[k1] < gList[k2] || valsS[k1] == G:
				result[i] = G
			case tList[k1] < tList[k2] || valsS[k1] == T:
				result[i] = T
			}
		}
	}
